...
@Component({
  selector: 'app-bin-two',
  templateUrl: './bin-two.component.html',
  styleUrls: ['./bin-two.component.css']
})
export class BinTwoComponent implements OnInit {
  range = new FormGroup({
  ...
  sensorData(){
    this.service.getLatest2().subscribe(res=>{
  ...
  loadData(){
    ...
    if(this.range.value.start==null || this.range.value.end==null){
      ...
      this.service.getSensorDate2(dateStart,dateEnd).subscribe(res=>{
        this.sensorList=res;
  ...
  getLogs(){
    this.service.getSensorData2().subscribe(data=>{
      this.logList=data;
